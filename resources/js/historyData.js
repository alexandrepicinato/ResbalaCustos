...
async function loaddata(buzzinesRequest, productRequest){
        ...
        var api=await fetch('http://localhost:8000/api/consultoria/acompanhamento/'+buzzinesRequest+'/'+productRequest+'?token='+tkgen(128)).then((resp)=> resp.json());
        var datalog =[]
        var timestamplog =[]
...
function tablemader(tableData){
    ...
    for (var i = 0; i < (dataTable.length ); i++) {
        ...
        price.textContent = dataTable[i]['currentprice']
